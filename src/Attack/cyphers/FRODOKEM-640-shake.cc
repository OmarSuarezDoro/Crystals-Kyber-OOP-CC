...
#include "./FRODOKEM-640-shake.h"

/**
 * @brief Construct a new Frodokem_640_shake::Frodokem_640_shake object
 */
Frodokem_640_shake::Frodokem_640_shake() {
  // First, we need to initialize the FRODOKEM-640-shake cypher
  kem = OQS_KEM_new(OQS_KEM_alg_frodokem_640_shake);
  if (kem == nullptr) {
    throw std::runtime_error("ERROR: Unable to initialize the Frodokem_640_shake cypher.");
  }
  ...
  if (status != OQS_SUCCESS) {
    OQS_KEM_free(kem);
    throw std::runtime_error("ERROR: Unable to generate the keypair for the Frodokem_640_shake cypher.");
  }
}
 ...
 */
std::pair<Bytes, Bytes> Frodokem_640_shake::Encrypt(const Bytes& pk) {
  ...
  // Encrypt the message
  OQS_STATUS status = OQS_KEM_frodokem_640_shake_encaps(ciphertext.data(), shared_secret.data(), aux_pk.data());
  if (status != OQS_SUCCESS) {
 ...
 */
Bytes Frodokem_640_shake::Decrypt(const Bytes& cyphertext) {
  std::vector<uint8_t> aux_cyphertext = cyphertext.GetBytes();
  std::vector<uint8_t> shared_secret(kem->length_shared_secret);
  // Decrypt the message
  OQS_STATUS status = OQS_KEM_frodokem_640_shake_decaps(shared_secret.data(), aux_cyphertext.data(), secret_key_.data());
  if (status != OQS_SUCCESS) {
